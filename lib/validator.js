...
function validateRepo(r, data, licenses, w3cgroups) {
  const {contributing, contributingSw, contributingCg, license, licenseSw, licenseCg} = licenses;
  ...
  if (conf) {
    ...
    if (!conf.group && conf["repo-type"] && hasIntersection(["rec-track", "note", "cg-report"], conf["repo-type"])) {
      ...
      // create normalized values to type/shortname
      conf.normalizedGroups = groups.map(id => w3cgroups.find(g => g.id === id).fullshortname);
  ...
  if (conf && conf["repo-type"] && hasIntersection(["rec-track", "note", "cg-report"], conf["repo-type"])) {
    if (!r.license || !r.license.text) {
      ...
      if (hasIntersection(["rec-track"], conf["repo-type"])) {
        ...
        if (!mdMatch(r.license.text, licenseCg)) {
          reportError('invalidlicense', {error: "doesn't match CG license", license: r.license.text});
        }
      }
    }
