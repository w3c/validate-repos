...
const w3cjsonSchema = {
  ...
  properties: {
    ...
    contacts: {
      type: 'array',
      items: {type: 'string', format: 'github-user'},
      minItems: 1,
    },
    'repo-type': {
      anyOf: [
        ...
        {
          type: 'array',
          items: {'$ref': '#/definitions/repoType'},
          minItems: 1,
        }
      ]
    },
...
ajv.addFormat('github-user', {
  ...
  async validate(username) {
    ...
    let found;
    try {
      found = !!(await github.getUser(username));
    } catch {
      found = false;
    }
    usernameCache.set(username, found);
    return found;
  }
});

const validateW3CJSON = ajv.compile(w3cjsonSchema);
...
function validateRepo(r, data, licenses) {
  ...
  if (conf) {
    r.w3c = conf;
    const valid = validateW3CJSON(conf);
    if (!valid) {
      for (const error of validateW3CJSON.errors) {
        reportError('invalidw3cjson', {error});
      }
    }
...
module.exports = {validateRepo, validateAshHooks, validateW3CJSON, ajv};
